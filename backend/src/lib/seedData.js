...
const users = [
  {
    ...
    role: 'admin',
    isDemo: true,
    locationSettings: { searchLocation: SF, nearMeRadius: 100 },
    currentCity: SF
  },
  {
  ...
  },
  {
    ...
    isDemo: true,
    locationSettings: { searchLocation: SR, nearMeRadius: 100 },
    currentCity: SR
  },
  {
...
/**
 * Wipes all demo seed data and recreates it fresh.
 * Returns the alex user doc (the demo login user).
 */
export async function reseedDemoData() {
  ...
  if (existingSeedUsers.length > 0) {
    ...
    await Promise.all([
      Post.deleteMany({ author: { $in: seedUserIds } }),
      Event.deleteMany({ creator: { $in: seedUserIds } }),
      Comment.deleteMany({ author: { $in: seedUserIds } }),
      Follow.deleteMany({ $or: [{ follower: { $in: seedUserIds } }, { following: { $in: seedUserIds } }] }),
      Song.deleteMany({ submittedBy: { $in: seedUserIds } }),
      Poll.deleteMany({ creator: { $in: seedUserIds } }),
      Friendship.deleteMany({ $or: [{ requester: { $in: seedUserIds } }, { recipient: { $in: seedUserIds } }] }),
      Message.deleteMany({ $or: [{ senderId: { $in: seedUserIds } }, { receiverId: { $in: seedUserIds } }] }),
    ]);
    // Remove seed users from other users' arrays
  ...
  const messages = [
    ...
    { senderId: jordan._id, receiverId: alex._id, text: 'Perfect. See you there \u{1F3B6}', createdAt: new Date(now - 3600000 * 2) },
  ...
  const posts = [
    {
      ...
      image: 'https://images.unsplash.com/photo-1566417713940-fe7c737a9ef2?w=1200&auto=format&fit=crop',
    ...
    },
    {
      ...
      image: 'https://images.unsplash.com/photo-1483412033650-1015ddeb83d1?w=1200&auto=format&fit=crop',
    ...
    },
    {
      ...
      image: 'https://images.unsplash.com/photo-1476480862126-209bfaa8edc8?w=1200&auto=format&fit=crop',
      likes: [maya._id]
    },
    {
  ...
  const comments = [
    ...
    { text: 'Can\'t wait for this one \u{1F3B6}', author: maya._id, parentType: 'post', parentId: createdPosts[0]._id, likes: [alex._id] },
